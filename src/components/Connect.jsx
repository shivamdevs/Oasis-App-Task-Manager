import { Input } from './Elements/Elements';
import css from './../styles/connect.module.css';
import { useState } from 'react';
import Pocketbase from 'pocketbase';

function Connect(props) {
    const client = new Pocketbase("http://127.0.0.1:8090");
    const [state, setState] = useState("login");
    const [email, setEmail] = useState("");
    const [emailError, setEmailError] = useState("");
    const [password, setPassword] = useState("");
    const [passwordError, setPasswordError] = useState("");
    async function login (e) {
        e.preventDefault();
        setEmailError("");
        setPasswordError("");
        e.target[2].disabled = true;
        let res = null;
        try {
            res = await client.users.authViaEmail(email, password);
        } catch (err) {
            console.log(err);
            setEmailError("Something doesn't seem to match");
            setPasswordError("Try again");
            e.target[2].disabled = false;
            e.target[0].focus();
            return;
        }
        setEmailError("");
        setPasswordError("");
        e.target[2].disabled = false;
        props.setAuthorised(res);
    }
    return (
        <div className={css.connector}>
            <div className={css.layout}>
                <div className={css.wrapper}>
                    {state === "login" && <>
                        <div className={css.content}>
                            <div className={css.header}>
                                <div className={css.symbol}>
                                    <img className={css.logo} src="/logo192.png" alt="" />
                                    <span className={css.text}>Task Manager</span>
                                </div>
                            </div>
                            <form action="" className={css.form} onSubmit={login}>
                                <Input
                                    type="email"
                                    label="Email Address"
                                    autoComplete="email"
                                    onChange={(e)=>setEmail(e.target.value)}
                                    error={emailError || ""}
                                    required={true}
                                />
                                <Input
                                    type="password"
                                    label="Password"
                                    autoComplete="current-password"
                                    onChange={(e)=>setPassword(e.target.value)}
                                    error={passwordError || ""}
                                    required={true}
                                />
                                <div className="justify-center">
                                    <button type="submit" className={css.button}>Sign in</button>
                                </div>
                            </form>
                        </div>
                        <div className={css.content}>
                            <div className="justify-center">
                                <button type="button" onClick={()=>setState("password")} className={css.happy}>
                                    <span>Forgot your Password?</span>
                                </button>
                            </div>
                            <br />
                            <div className="justify-center">
                                <button type="button" onClick={()=>setState("register")} className={css.happy}>
                                    <span>Create a new Account</span>
                                </button>
                            </div>
                        </div>
                    </>}
                    {state === "register" && <>
                        <div className={css.content}>
                            <div className={css.header}>
                                <div className={css.symbol}>
                                    <img className={css.logo} src="/logo192.png" alt="" />
                                    <span className={css.text}>Task Manager</span>
                                </div>
                            </div>
                            <form action="" className={css.form}>
                                <Input
                                    type="text"
                                    label="Full Name"
                                />
                                <Input
                                    type="email"
                                    label="Email Address"
                                />
                                <Input
                                    type="password"
                                    label="Password"
                                />
                                <div className="justify-center">
                                    <button type="submit" className={css.button}>Sign up</button>
                                </div>
                            </form>
                            <div className="justify-center">
                                <button type="button" className={css.happy}>
                                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className={css.google}>
                                        <g>
                                            <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"></path>
                                            <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"></path>
                                            <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"></path>
                                            <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"></path>
                                            <path fill="none" d="M0 0h48v48H0z"></path>
                                        </g>
                                    </svg>
                                    <span>Continue with Google</span>
                                </button>
                            </div>
                        </div>
                        <div className={css.content}>
                            <div className="justify-center">
                                <button type="button" onClick={()=>setState("login")} className={css.happy}>
                                    <span>Already have an Account? Sign in</span>
                                </button>
                            </div>
                        </div>
                    </>}
                    {state === "password" && <>
                        <div className={css.content}>
                            <div className={css.header}>
                                <div className={css.symbol}>
                                    <img className={css.logo} src="/logo192.png" alt="" />
                                    <span className={css.text}>Task Manager</span>
                                </div>
                            </div>
                            <form action="" className={css.form}>
                                <Input
                                    type="email"
                                    label="Email Address"
                                />
                                <div className="justify-center">
                                    <button type="submit" className={css.button}>Send Password Reset Link</button>
                                </div>
                            </form>
                        </div>
                        <div className={css.content}>
                            <div className="justify-center">
                                <button type="button" onClick={()=>setState("login")} className={css.happy}>
                                    <span>Back to Sign in</span>
                                </button>
                            </div>
                        </div>
                    </>}
                </div>
            </div>
        </div>
    );
}
export default Connect;